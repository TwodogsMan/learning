...
def format_player_list(players: List[Any], show_roles: bool = False) -> str:
    """格式化玩家列表为中文显示"""
    if not players:
        return "无玩家"
...
def majority_vote_cn(votes: Dict[str, Optional[str]]) -> tuple:
    """中文版多数投票统计"""
    if not votes:
        return "无人", 0
...
def check_winning_cn(alive_players: List[Any], roles: Dict[str, str]) -> Optional[str]:
    """检查中文版游戏胜利条件"""
...
class GameModerator:
    ...
    def _make_msg(self, content: str, role: str = "user") -> Msg:
        """创建游戏消息"""
        return Msg(
            name=self.name,
    ...
    async def announce(self, content: str) -> Msg:
        ...
        msg = self._make_msg(full_content, role="user")
